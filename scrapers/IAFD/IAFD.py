...
# IAFD is kept in /scrapers/my so CommunityScrapers updates do not overwrite it.
COMMUNITY_DIR = Path(__file__).resolve().parents[2] / "community"
...
def performer_circumcised(tree):
    return maybe(
        tree.xpath(
            '//p[@class="bioheading" and normalize-space(text())="Penis"]'
...
def scrape(url: str, retries=0):
    try:
        ...
        if retries < 5:
            ...
            log.warning(
                f"Network error: {e}, retry {retries + 1}/5 "
                f"in {wait_time} seconds"
            )
            time.sleep(wait_time)
            return scrape(url, retries + 1)

        log.error(f"Network error after 5 retries: {e}")
        sys.exit(1)
    ...
    if scraped.status_code >= 400:
        if retries < 10:
            wait_time = random.randint(1, 4)
            log.debug(
                f"HTTP Error: {scraped.status_code}, "
...
def performer_from_tree(tree):
    ...
    return {
        ...
        "circumcised": performer_circumcised(tree),
