#!/usr/bin/env python3
"""ScrapeDiscovery - the plugin entry point.

Stash spawns this process for three different reasons and tells them apart only by the
arguments on stdin:
...
Whatever happens, exactly one JSON object goes to stdout, because that is how Stash
reads the result: `{"output": ...}` on success, `{"error": "..."}` on failure. Anything
else printed there corrupts the response, so all human-facing output goes to stderr
through logs.py.

The discovery logic itself lives in the `scrapediscovery` package; this file only works
out what was asked, opens the database, and hands over.
"""
...
from scrapediscovery import logs, ops, settings, stash, tasks  # noqa: E402
from scrapediscovery.db import repo as repository  # noqa: E402
...
def build_context(payload):
    ...
    url, api_key, cookie, config_dir, _plugin_dir = stash.from_plugin_input(payload)
    note = None
    if not (payload or {}).get("server_connection"):
        # Run by hand rather than by Stash: fall back to the environment, so the
        # package stays testable outside a Stash process.
        import os
...
def main() -> int:
    ...
    try:
        ...
        from scrapediscovery.executor import describe_error
        message = describe_error(exc)
        logs.error("ScrapeDiscovery failed: " + message)
        # A plugin-level error, which surfaces to the caller as a GraphQL error. Only
        # for a failure that makes the plugin unusable - an operation that merely could
        # not do what was asked reports that inside its own output.
