...
If a column clearly matched the wrong scene, tick **reject** in its header. Everything it
said drops out of every row at once — values, votes, performers, covers — and a row it was
the only contributor to disappears. The column stays in the table, dimmed, so you can
untick it and get it all back.

It is per column rather than per source on purpose: a stash-box searched by title answers
with a list, and you get `StashDB (search)` and `StashDB (search) #2` as separate columns
precisely because the first can be your scene and the second a different film. The choice
is remembered until you decide, and Apply writes against exactly the table you were
looking at.
